...
function setup() {
  ...
  instructions.push(tempObj); //instructions[4]
};
...
function draw() { // runs once every frame
  ...
  if (gamestate1 == -1) {
    ...
    settings[0].option = advancedPunctuation;
    settings[1].option = advancedCapitals;
  ...
  if (gamestate1 == 2.12 || gamestate1 == 2.14) {
    amountSelector[0].show();//show ammount selector on adjective sentences and random menu
  };
  ...
  if (gamestate1 == 2.11) {
    ...
    if (pageCounter + 1 < sentenceSelect.length / 26) {// instead of containing incrementing next page, showing the button is now also contained
      nav[6].show();
    ...
    for (let i = 0; i < customSentenceToggle.length; i++) {
      ...
      customColor = 0;
      if (customSentenceToggle[i].toggleClickCheck() == 1) {//if sentence sorces changed
        initialiseButtons();
      };
    };
    ...
    for (let i = 0; i < filterSelect.length; i++) {
      ...
      customColor = 0;
      if (filterSelect[i].toggleClickCheck() == 1) {//if filters changed
        initialiseButtons();
      };
    ...
    while (posCounter < 26) {
      if (sentenceCounter < sentenceSelect.length) {
        sentenceSelect[sentenceCounter].showTag();  //showTag checks if mouse is over and then shows a preset dialoge over mouse
      };
      ...
      if (sentenceCounter < adjectiveSelect.length - 1) {
        ...
        if (adjectiveSelect[sentenceCounter].singleClickCheck() == 1) {
          ...
          for (let j = 0; j < amountSelector[0].posNum + 1; j++) {
            ...
            sentenceQueue.push(adjectiveSentences[sentenceCounter * 2].replace("_", adjectives[tempRandom]));// replace underscore with adjective and push button to array
  ...
  if (gamestate1 == 2.9) {//custom sentences
    ...
    i++;
    sentenceButtons[i].show();
    if (sentenceButtons[i].singleClickCheck() == 1 || info[6].singleClickCheck()) { //if filter button clicked
      sentenceButtons[i].next(); //next filter
    };
    i++;
    ...
    if (customSelection[0].singleClickCheck() == 1) {
      ...
      i++;
      customSentence = customSentence + " " + sentenceButtons[i].tex; //add filter
      i++;
  ...
  if (gamestate1 >= 4 && gamestate1 < 5) {
    ...
    if (attempts < 3 && check < sentenceLength) { //if run out of attempts and words are in correct place
      instructions[2].show();
    };
    ...
    if ((check >= sentenceLength) || (stasis == 1 && attempts < 4 && attempts > 3.5)) { //when sentence is correct or max attempts reached
      if (advancedCapitals == 1 && attempts != 4) {
        for (let i = 0; i < words.length - advancedPunctuation; i++) {
          if (words[i].singleClickCheck() == 1) { //if word pressed
  ...
  if (gamestate1 == 5.1) {
    ...
    for (let i = 0; i < finalQueue.length; i++) {
      ...
      console.log(tempX, tempY, finalQueue[i], sentenceQueue[i], scoreQueue[i], i)
  ...
  if (gamestate1 == 5.2) {
    ...
    if (pageCounter + 1 < moreFeedback.length / 6) {// if there is a second page
      nav[6].show();
      if (nav[6].singleClickCheck() == 1) {//if next page pressed
        pageCounter++; //next page
    ...
    if (pageCounter > 0) {// if not on the first page
      nav[7].show();
      if (nav[7].singleClickCheck() == 1) {// same but going back pages
        pageCounter = pageCounter - 1;
      };
...
function selectFont() {
  ...
  updateFont();//change fonts and adjust sizes
...
function resetVisuals() {
  ...
  instructions[0].move(200, 100)
  instructions[1].move(200, 100)
  instructions[2].move(windowWidth / 2 - 500, 10)
  instructions[3].move(windowWidth / 2 - 210, windowHeight / 2 - 200)
  instructions[4].move(windowWidth / 2 - 500, 10)
};
...
function nextSentence() {
  ...
  if (sentencePos > 0) {
    ...
    while (i < sentenceLength) {
      console.log(words[i].correct, words[i].correctCapital);
  ...
  if (sentencePos < sentenceQueue.length) {//if there is another sentence
    ...
    for (let i = 0; i < sentenceLength + advancedPunctuation; i++) { //add extra box if advanced assembly is on
      ...
      if (i < sentenceLength - 1) {
        tempWord = new word(windowWidth + 200, random(50, windowHeight - 400), i, sentence[i], 180, 90, Math.round(random(1, 4)), capital);//create objects for words
        destination = new tile(pos, windowHeight + 30, i, 180, 90);//create objects for word, destinations
      };
      if (i == sentenceLength - 1) {
        tempWord = new word(windowWidth + 200, random(50, windowHeight - 400), i, sentence[i].substring(0, sentence[i].length - advancedPunctuation), 180, 90, Math.round(random(1, 4)), capital); // if advanced assembled enabled, remove last character of last word (punctuation)...
        destination = new tile(pos, windowHeight + 30, i, 180, 90);
      };
      if (i == sentenceLength) {
        tempWord = new word(windowWidth + 200, random(50, windowHeight - 400), i, sentence[i - 1].slice(-1), 180, 90, 0, capital); //...and make it its own object
        destination = new tile(pos, windowHeight + 30, i, 180, 90);
      };
      ...
      words.push(tempWord);
      words[i].goTo(random(0, windowWidth - 200), random(50, windowHeight - 400), 100);
      if (advancedCapitals == 1) {
...
function lengthInsertionSort(input) {
  for (let i = 1; i < input.length; i++) {//for every item in array
    key = input[i];//isolate value to compare
    let j = i - 1;
    while (j >= 0 && input[j].length > key.length) {//compare from end downwards until value is smaller
      input[j + 1] = input[j];
      j--;
    };
    input[j + 1] = key;
...
function initialiseButtons() {
  ...
  let tempLength;
  for (let i = 0; i < filterSelect.length; i++) {
    if (filterSelect[i].option == 1) {//add up all filters
      filterNum++;
    };
  };
  for (let i = 0; i < customSentenceToggle.length; i++) {
    if (customSentenceToggle[i].option == 1) {// add up all sentence sorces
      inputNum++;
    };
  };
  for (i = 0; i < filterSelect.length; i++) {
    if ((filterSelect[i].option == 1 || filterNum == 0) && (customSentenceToggle[1].option == 1 || inputNum == 0)) {// if premade sentences and filter is on. or is none is selected, queue coresponding sentences
    ...
    if ((filterSelect[i].option == 1 || filterNum == 0) && (customSentenceToggle[0].option == 1 || inputNum == 0)) {// if custom sentences and filter is on. or is none is selected, queue coresponding sentences
